...
titles = [] #a list of course titles in the same order as courses
...
for course in coursesByNames:
    titles.append(coursesByNames[course].title)

class DataPoint:
    def __init__(self, gradYear, stuID, stuGen, stuSem, stuMaj, courseID, profID, coursetitle):
...
def createDataPoints():
    ...
    for row in fileContents:
        ...
        if type(attrs)==list:
            ...
            dp = DataPoint(gradYear, stuID, stuGen, stuSem, stuMaj, courseID, profID, coursetitle)
            allData.append(dp)
    return allData
